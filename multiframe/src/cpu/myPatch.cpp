...
void gen_gpu() {
    ...
    const std::string guide_name = "video/gen/";//"video/guides/";
    int key_cnt = 99;
    int* nnf = nullptr;
    ...
    std::string A_start_path[5] = { src_name + "000.jpg",
                              guide_name + "edge/" + "edge0.jpg",
                              guide_name + "pos/" + "pos0.jpg",
                              src_name + "000.jpg",
                              "video/img/mask/000.jpg" };
    std::string A_end_path[5] = { src_name + "099.jpg",
                              guide_name + "edge/" + "edge99.jpg",
                              guide_name + "pos/" + "pos99.jpg",
                              src_name + "000.jpg",
    ...
    gaussian_pyramid(A_start_prime, gaussian_levels, A_pre_prime_pyramids);
    gaussian_pyramid(A_start_merge, gaussian_levels, A_pre_pyramids);
    ...
    for (int i = 1; i < key_cnt; i++) {
        ...
        if (i % 2 == 1) {
            if (i < DIVIDER) {
                ...
                gaussian_pyramid(A_end_prime, gaussian_levels, A_pre_prime_pyramids);
                gaussian_pyramid(A_end_merge, gaussian_levels, A_pre_pyramids);
            }
        }
        ...
        std::string B_path[5] = { src_name + "0" + (i < 10 ? ("0" + index) : index) + ".jpg",
                                  guide_name + "edge/edge" + index + ".jpg",
                                  guide_name + "pos/pos" + index + ".jpg",
                                    "video/guides/temp" + index + ".png",
                                    //"video/img/mask/0" + (i < 10 ? ("0" + index) : index) + ".jpg" };
                                    guide_name + "mask/" + index + ".jpg" };
        ...
        // the first frame
        if (pre_nnf == nullptr || i == DIVIDER || i%2 == 1) {
        ...
        apply_frame_pyramids(i, A_start_prime, A_end_prime, B_prime_pyramids[0], A_pre_prime_pyramids, pre_nnf);
...
void apply_frame_pyramids(int i, cv::Mat& A_start_prime, cv::Mat& A_end_prime, cv::Mat& b_prime, std::vector<cv::Mat>& B_prime_pyramids, int* nnf) {
    int B_width = b_prime.cols;
    int B_height = b_prime.rows;
    ...
    for( int j = 0; j < B_height; j++)
        for (int k = 0; k < B_width; k++) {
            ...
            if (i < DIVIDER) {
                ...
                b_prime.at<Vec3b>(j, k) = A_end_prime.at<Vec3b>(nnf[2 * idx], nnf[2 * idx + 1]);
            }
        }
    gaussian_pyramid(b_prime, gaussian_levels, B_prime_pyramids);
}