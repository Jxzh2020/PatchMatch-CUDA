...
__device__ float patch_distance(float* a, float* b, int A_width, int B_width, int channels, int patch_height, int patch_width, float* u, int isPrime) {
    float dist = 0.f;
    float dis_one = 0.f;
    float tmp;
    for (int i = 0; i < patch_height; i++) {
        for (int j = 0; j < patch_width; j++)
            for (int k = 0; k < channels; k++) {
                ...
                if(isPrime)
                    ...
                    dist += u[k/3] * fabs(a[(i * A_width + j) * channels + k] - b[(i * B_width + j) * channels + k]);
                }
            }


    }
    return dist / (patch_height * patch_width * channels);
}
...
__global__ void initialize_nnf(int* nnf, int A_width, int A_height, int B_width, int B_height, int patch_size, int seed) {
    ...
    if (x < A_height && y < A_width ) {
        ...
        int dx = (int)(curand_uniform(&state) * (B_height));
        int dy = (int)(curand_uniform(&state) * (B_width));

        nnf[2 * (x * A_width + y)] = dx;//x + dx >= 0 && x + dx < B_height ? x + dx : x;
        nnf[2 * (x * A_width + y) + 1] =  dy; // y + dy >= 0 && y + dy < B_width ? y + dy : y;
    }
}

__global__ void apply_nnf(float* dev_a_prime, float* dev_b_prime,
                          int A_width, int A_height, int B_width, int B_height, int channels, int patch_size, float* u, int iteration, const int* nnf) {
    ...
    x_max = x_max > 2 ? 2 : x_max;
    y_max = y_max > 2 ? 2 : y_max;
    average_patch(&dev_b_prime[(target_idx - x_min * B_width - y_min) * PRIME_CHANNELS], B_width, PRIME_CHANNELS, x_min + x_max, y_min + y_max, rgb);*/
    //average_patch(&dev_b_prime[(target_idx) * PRIME_CHANNELS], B_width, PRIME_CHANNELS, 1, 1, rgb);
    ...
    if (iteration == 0) {
        ...
        for (int i = 0; i < PRIME_CHANNELS; i++)
            dev_a_prime[idx * PRIME_CHANNELS + i] = (dev_a_prime[idx * PRIME_CHANNELS + i] * (RATIO - 1) + rgb[i]) / RATIO;
    }
}
...
__global__ void compute_patch_distances(float* a, float* b, float* dev_a_prime, float* dev_b_prime,
                                        int A_width, int A_height, int B_width, int B_height, int channels, int patch_size, float* u, const int* nnf, float* distances) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    a_b_dist = patch_distance(&a[(idx - x_min*A_width - y_min)*channels], &b[(target_idx - x_min*B_width-y_min)*channels], A_width, B_width, channels, x_min + x_max,y_min + y_max, u, false);
    a_b_prime_dist = patch_distance(&dev_a_prime[(idx - x_min * A_width - y_min) * PRIME_CHANNELS], &dev_b_prime[(target_idx - x_min * B_width - y_min) * PRIME_CHANNELS], A_width, B_width, PRIME_CHANNELS, x_min + x_max, y_min + y_max, u, true);

    distances[x * A_width + y] = a_b_dist * a_b_dist + a_b_prime_dist * a_b_prime_dist;
}
    ...
    if (!reversed) {
        if (y > 0) {
            ...
            if (b_left_y < B_width) {
                ...
                tmp_dist = patch_distance(&a[(idx - x_min * A_width - y_min) * channels], &b[(target_idx - x_min * B_width - y_min) * channels], A_width, B_width, channels, x_min + x_max, y_min + y_max, u, false);
                tmp_prime_dist = patch_distance(&dev_a_prime[(idx - x_min * A_width - y_min) * PRIME_CHANNELS], &dev_b_prime[(target_idx - x_min * B_width - y_min) * PRIME_CHANNELS], A_width, B_width, PRIME_CHANNELS, x_min + x_max, y_min + y_max, u, true);
                tmp_dist = tmp_dist * tmp_dist + tmp_prime_dist * tmp_prime_dist;
                if (dist > tmp_dist) {
                    dist = tmp_dist;
                    best_x = b_left_x;
                    best_y = b_left_y;
...
__global__ void random_search(float* a, float* b, float* dev_a_prime, float* dev_b_prime, int A_width, int A_height, int B_width, int B_height,
    ...
    for ( ; offset_radius > 1; offset_radius /= 2 ) {
        ...
        if (!(dx >= 0 && dx < B_height && dy >= 0 && dy < B_width)) {
            ...
            tmp_dist = patch_distance(&a[(idx - x_min * A_width - y_min) * channels], &b[(target_idx - x_min * B_width - y_min) * channels], A_width, B_width, channels, x_min + x_max, y_min + y_max, u, false);
            tmp_prime_dist = patch_distance(&dev_a_prime[(idx - x_min * A_width - y_min) * PRIME_CHANNELS], &dev_b_prime[(target_idx - x_min * B_width - y_min) * PRIME_CHANNELS], A_width, B_width, PRIME_CHANNELS, x_min + x_max, y_min + y_max, u, true);
            tmp_dist = tmp_prime_dist * tmp_prime_dist + tmp_dist * tmp_dist;
            if (tmp_dist < dist) {
                dist = tmp_dist;
                distances[idx] = dist;
                nnf[2 * idx] = dx;
