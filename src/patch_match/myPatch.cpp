...
void gen_gpu() {
    ...
    std::string A_path[5] = { "patch_match_src/files/sphere/rendered.png", "patch_match_src/files/sphere/l_dde.png", "patch_match_src/files/sphere/ld12e.png", "patch_match_src/files/sphere/lde.png", "patch_match_src/files/sphere/lse.png" };
    std::string B_path[5] = { "patch_match_src/files/" + name[c] + "/rendered.png", "patch_match_src/files/" + name[c] + "/l_dde.png", "patch_match_src/files/" + name[c] + "/ld12e.png", "patch_match_src/files/" + name[c] + "/lde.png", "patch_match_src/files/" + name[c] + "/lse.png"};
    std::string style[6] = { "draw", "victor", "comics", "dreamy", "orange_aqua", "fragments"};
    std::cout << "\n1: draw\n2: victor\n3: comics\n4: dreamy\n5: orange_aqua\n6: fragments" << std::endl;
    std::cin >> d;
    cv::Mat A_prime = imread("patch_match_src/files/sphere/" + style[d-1] + ".png"); //victor.png");//comics.png"); // "img/source_fullgi.png");// orange_aqua
    cv::Mat B_prime = imread("patch_match_src/files/" + name[c] + "/rendered.png");
    ...
    for (int i = 0; i < pics_num; i++) {
        imwrite("patch_match_src/output/A_c" + std::to_string(i) + ".jpeg", A[i]);
        imwrite("patch_match_src/output/B_c" + std::to_string(i) + ".jpeg", B[i]);
    }
    imwrite("patch_match_src/output/A_prime.jpeg", A_prime);
    imwrite("patch_match_src/output/B_prime.jpeg", B_prime_pyramids[0]);
